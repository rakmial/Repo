...
ggplot(aes(x=quality),data=wwines)+
  geom_histogram(binwidth=1)+
  scale_x_continuous(limits=c(2,10),breaks=3:9)+
  scale_y_sqrt()+
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
wwines$rating<- ifelse(wwines$quality<=4,'Poor',wwines$quality)
wwines$rating<- ifelse(wwines$rating %in% c(5,6),'Fair',wwines$rating)
wwines$rating<- ifelse(wwines$quality>=7,'Good',wwines$rating)
...
library(corrgram)
library(psych)
cor(wwines[2:13],use='pairwise.complete.obs',method='pearson')
pairs.panels(wwines[2:13])
```
...
* Free Sulfur Dioxide and Total Sulfur Dioxide
    ...
    These variables are certainly linked. It may be useful to create a new variable of their ratio.
...
A closer look at our quality-related scatterplots and boxplots is in order.
...
As the two individual independent variables with the greatest correlation coefficients, Alcohol and Density deserve special attention. 
...
```{r echo=FALSE,message=FALSE,warning=FALSE}
library(gridExtra)
ggplot(aes(x=sulfur.ratio, y=volatile.acidity,color=rating), data=wwines)+
  geom_point(alpha=.2,color='red')+
  geom_density_2d(color='blue',linetype=1)+
  scale_y_continuous(limits=c(0,0.9))+
  scale_x_continuous(limits=c(0,0.5))+
  facet_wrap(~rating)+
  ggtitle('Sulfur Ratio and Volatile Acidity Across Ratings featuring Density Contour')
...
ggplot(aes(x=alcohol,y=residual.sugar,color=quality),data=wwines)+
...
ggplot(aes(x=quality,y=alcohol,color=rating),data=wwines)+
  geom_boxplot(size=3)+
  geom_jitter(width=1)+
  stat_summary(fun.y=mean, colour='black', geom='line',linetype=2, size = 2)+
  scale_x_continuous(breaks=3:9)+
...
The first plot demonstrates the subtle effects of volatile acidity (the acidity category with greatest effect on the dependent variable) and sulfur ratio. Density contours aid in seeing the distribution of data points. In particular, it is apparent that volatile acidity above 0.5 is represented proportionally well only among 'Poor' wines. We may also see that the 'nuclei' of the density contours tend, across ratings, to increase in sulfur ratio and decrease in volatile acidity.

The second plot is a lesson in spurious correlation - I had created this plot before running the fourth model that includes every independent variable. The smoothers help us to see the difference in covariation between sulphates and chlorides between wines of Poor and Good quality, with the difference in means between the two ratings along the y-axis seeming pretty substantial. However, in light of the full model, chlorides failed to be a statistically significant predicting variable.

The third plot shows another side of the predictive quality of alcohol content. The whole of the graph shows a clear right-triangular shape - as we explored in our panel pairs, there is a solid negative correlation between residual sugar and alcohol content. This plot also shows some of the internal structure of the triangle by coloring points by their quality scores. Along the left, vertical edge of the triangle, most of the points are a dark blue, indicating a low score in quality. However, as alcohol increases, tolerance for residual sugar seems to increase - even points high in the Y-axis become lighter as we travel to the right on the X-axis.

However, our linear model demonstrates one thing for certain - the majority of the predictive power of our independent variables comes from the alcohol variable. In our final plot, we may pick out a few different measures of the effect of alcohol on quality.
